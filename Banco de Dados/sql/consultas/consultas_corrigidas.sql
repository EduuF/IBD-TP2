...
SELECT DISTINCT(titulo_filme),nota FROM filmes NATURAL JOIN filme_genero NATURAL JOIN generos NATURAL JOIN avaliacoes WHERE nome_genero = 'Romance' AND nota = (SELECT MAX(nota) FROM avaliacoes) ORDER BY titulo_filme;
